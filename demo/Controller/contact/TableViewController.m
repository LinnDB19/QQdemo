        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if(self.indexPath)
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    WEAKSELF(weakSelf)
                    weakSelf.moveCell = [weakSelf.tableView cellForRowAtIndexPath:weakSelf.indexPath];
                    weakSelf.snapView = [weakSelf.moveCell snapshotViewAfterScreenUpdates:NO];
                    weakSelf.snapView.frame = weakSelf.moveCell.frame;
                    [weakSelf.tableView addSubview:weakSelf.snapView];  //显示要移动的cell的快照
                    weakSelf.moveCell.hidden = YES;
                    [UIView animateWithDuration:0.1 animations:^{
                        WEAKSELF(weakSelf)
                        weakSelf.snapView.transform = CGAffineTransformScale(weakSelf.snapView.transform, 1.03, 1.05);
                        weakSelf.snapView.alpha = 0.8;
                    }];
                });
            }
        }
...
{
    Person *thePerson =(Person *)([(Group *)self.groups[indexPath.section] PersonAtIndex:(int)indexPath.row]);
    CusViewController *vc = [CusViewController new];
    vc.friendImgName = thePerson.photoName;
    vc.friendName = thePerson.nickName;
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
