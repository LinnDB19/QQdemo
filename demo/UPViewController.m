...
@interface UPViewController ()<UITextFieldDelegate, MNXNumberKeyBoardViewDelegate>
...
@property MNXNumberKeyboardView *numberKeyboard;    //the defined numberPad
...
- (void)viewDidLoad
{
    ...
    self.numberKeyboard = [[MNXNumberKeyboardView alloc] init];
...
- (void)keyboard:(MNXNumberKeyboardView *)keyboard didClickButton:(UIButton *)textBtn
{
    NSString *str = textBtn.titleLabel.text;
    NSMutableString *text = [NSMutableString stringWithString: self.idCardNumberSelfDefine.text];
    if([str isEqualToString:@"delete"])
    {
        if(text != nil && text.length)
        {
            [text deleteCharactersInRange: NSMakeRange(text.length - 1, 1)];
            self.idCardNumberSelfDefine.text = [NSString stringWithString:text];
        }
    }
    else
    {
        if(text == nil) self.idCardNumberSelfDefine.text = str;
        else
        {
            [text appendString:str];
            self.idCardNumberSelfDefine.text = text;
        }
    }
}
